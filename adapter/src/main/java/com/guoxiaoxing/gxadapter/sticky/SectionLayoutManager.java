...
public abstract class SectionLayoutManager {
    ...
    protected StickyLayoutManager mStickyLayoutManager;

    public SectionLayoutManager(StickyLayoutManager stickyLayoutManager) {
        mStickyLayoutManager = stickyLayoutManager;
    }
    ...
    public int findFirstCompletelyVisibleItemPosition(int sectionFirstPosition) {
        ...
        if (item == null) {
            return StickyLayoutManager.INVALID_POSITON;
        }
        return mStickyLayoutManager.getPosition(item);
    }
    ...
    public StickyLayoutManager.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        return new StickyLayoutManager.LayoutParams(c, attrs);
    }
    ...
    public View getFirstCompletelyVisibleView(int sectionFirstPosition, boolean skipHeader) {
        final int topEdge = mStickyLayoutManager.getClipToPadding() ? mStickyLayoutManager.getPaddingTop() : 0;
        final int bottomEdge = mStickyLayoutManager.getClipToPadding() ?
                mStickyLayoutManager.getHeight() - mStickyLayoutManager.getPaddingBottom() :
                mStickyLayoutManager.getHeight();

        int lookAt = 0;
        int childCount = mStickyLayoutManager.getChildCount();
        View candidate = null;
        while (true) {
            ...
            final View view = mStickyLayoutManager.getChildAt(lookAt);

            final boolean topInside = mStickyLayoutManager.getDecoratedTop(view) >= topEdge;
            final boolean bottomInside = mStickyLayoutManager.getDecoratedBottom(view) <= bottomEdge;

            StickyLayoutManager.LayoutParams lp = (StickyLayoutManager.LayoutParams) view.getLayoutParams();
    ...
    public int getHighestEdge(int sectionFirstPosition, int firstIndex, int defaultEdge) {
        ...
        for (int i = firstIndex; i < mStickyLayoutManager.getChildCount(); i++) {
            View child = mStickyLayoutManager.getChildAt(i);
            ...
            return mStickyLayoutManager.getDecoratedTop(child);
        }
        return defaultEdge;
    }
    ...
    public View getLastCompletelyVisibleView(int sectionFirstPosition) {
        ...
        int lookAt = mStickyLayoutManager.getChildCount() - 1;
        View candidate = null;
        while (true) {
            if (lookAt < 0) {
                return candidate;
    ...
    public View getLastVisibleView(int sectionFirstPosition) {
        ...
        while (true) {
            ...
            View view = mStickyLayoutManager.getChildAt(lookAt);
            StickyLayoutManager.LayoutParams lp = (StickyLayoutManager.LayoutParams) view.getLayoutParams();
            if (sectionFirstPosition == lp.getTestedFirstPosition()) {
                if (!lp.isHeader) {
                    return view;
    ...
    public int getLowestEdge(int sectionFirstPosition, int lastIndex, int defaultEdge) {
        ...
        for (int i = lastIndex; i >= 0; i--) {
            ...
            // A more interesting layout would have to do something more here.
            return mStickyLayoutManager.getDecoratedBottom(child);
        }
        return defaultEdge;
    }
    ...
    protected int addView(LayoutState.View child, int position, StickyLayoutManager.Direction direction,
            LayoutState state) {
        int addIndex;
        if (direction == StickyLayoutManager.Direction.START) {
            addIndex = 0;
        } else {
            addIndex = mStickyLayoutManager.getChildCount();
        }

        state.decacheView(position);
        mStickyLayoutManager.addView(child.view, addIndex);

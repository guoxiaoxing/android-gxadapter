 ...
 */
public class GridStickyLayoutManager extends SectionLayoutManager {

    private static final int AUTO_FIT = -1;

    private static final int DEFAULT_NUM_COLUMNS = 1;

    public static int ID = StickyLayoutManager.SECTION_MANAGER_GRID;
    ...
    public GridStickyLayoutManager(StickyLayoutManager stickyLayoutManager, Context context) {
    ...
    public int computeHeaderOffset(int firstVisiblePosition, SectionData sd, LayoutState state) {
        ...
        for (int position = sd.firstPosition + 1;
            ...
            for (int col = 0; col < mNumColumns && position + col < itemCount; col++) {
                ...
                rowHeight =
                        Math.max(rowHeight, mStickyLayoutManager.getDecoratedMeasuredHeight(child.view));
    ...
    public int fillToEnd(int leadingEdge, int markerLine, int anchorPosition, SectionData sd,
        ...
        for (int i = 1; i <= col; i++) {
            ...
            for (int j = 1; j <= mStickyLayoutManager.getChildCount(); j++) {
                View child = mStickyLayoutManager.getChildAt(mStickyLayoutManager.getChildCount() - j);
                if (mStickyLayoutManager.getPosition(child) == anchorPosition - i) {
                    markerLine = mStickyLayoutManager.getDecoratedTop(child);
                    mStickyLayoutManager.detachAndScrapViewAt(j, state.recycler);
                    break;
                }

                StickyLayoutManager.LayoutParams params = (StickyLayoutManager.LayoutParams) child
                        .getLayoutParams();
        ...
        for (int i = anchorPosition; i < itemCount; i += mNumColumns) {
            ...
            int rowHeight = fillRow(markerLine, i, StickyLayoutManager.Direction.END, true, sd, state);
    ...
    public int fillToStart(int leadingEdge, int markerLine, int anchorPosition, SectionData sd,
        ...
        for (int i = 0; i < mStickyLayoutManager.getChildCount(); i++) {
        ...
        for (int i = 1; i < mNumColumns - col; i++) {
            // Detach and scrap attached items in this row, so we can re-lay them again. The last
            // child view in the index can be the header so we just skip past it if it last.
            for (int j = 0; j < mStickyLayoutManager.getChildCount(); j++) {
                View child = mStickyLayoutManager.getChildAt(j);
                StickyLayoutManager.LayoutParams params = (StickyLayoutManager.LayoutParams) child
                        .getLayoutParams();
                if (params.getTestedFirstPosition() != sd.firstPosition) {
                    break;
        ...
        if (applyMinHeight) {
            for (int i = columnAnchorPosition; i >= 0; i -= mNumColumns) {
                ...
                StickyLayoutManager.LayoutParams checkParams = check.getLayoutParams();
                if (checkParams.getTestedFirstPosition() != sd.firstPosition) {
                    break;
                }
                ...
                for (int j = 0; j < mNumColumns && i + j <= anchorPosition; j++) {
                    ...
                    StickyLayoutManager.LayoutParams measureParams = measure.getLayoutParams();
                    ...
                    rowHeight = Math.max(rowHeight,
                            mStickyLayoutManager.getDecoratedMeasuredHeight(measure.view));
                }
        ...
        for (int i = columnAnchorPosition; i >= 0; i -= mNumColumns) {
            ...
            StickyLayoutManager.LayoutParams params = rowAnchor.getLayoutParams();
            ...
            int rowHeight = fillRow(markerLine, i, StickyLayoutManager.Direction.START, measureRowItems,
                    sd, state);
            markerLine -= rowHeight;
        }
    ...
    public int finishFillToEnd(int leadingEdge, View anchor, SectionData sd, LayoutState state) {
        ...
        final int markerLine = getLowestEdge(sd.firstPosition, mStickyLayoutManager.getChildCount() - 1,
                mStickyLayoutManager.getDecoratedBottom(anchor));
    ...
    @Override
    public StickyLayoutManager.LayoutParams generateLayoutParams(StickyLayoutManager.LayoutParams params) {
        return LayoutParams.from(params);
    }

    @Override
    public StickyLayoutManager.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
    ...
    public int getLowestEdge(int sectionFirstPosition, int lastIndex, int defaultEdge) {
        ...
        int leftPosition = mStickyLayoutManager.getWidth();
        boolean foundItems = false;
        // Look from end to find children that are the lowest.
        for (int i = lastIndex; i >= 0; i--) {
            View look = mStickyLayoutManager.getChildAt(i);
            StickyLayoutManager.LayoutParams params = (StickyLayoutManager.LayoutParams) look.getLayoutParams();
            ...
            foundItems = true;
            bottomMostEdge = Math.max(bottomMostEdge, mStickyLayoutManager.getDecoratedBottom(look));
        }

        return foundItems ? bottomMostEdge : defaultEdge;
    }

    public GridStickyLayoutManager init(SectionData sd) {
        super.init(sd);
     ...
     */
    public int fillRow(int markerLine, int anchorPosition, StickyLayoutManager.Direction direction,
        ...
        for (int i = 0; i < mNumColumns; i++) {
            ...
            rowHeight = Math.max(rowHeight, mStickyLayoutManager.getDecoratedMeasuredHeight(view.view));
            views[i] = view;
        }

        boolean directionIsStart = direction == StickyLayoutManager.Direction.START;
        if (directionIsStart) {
            markerLine -= rowHeight;
        }
    ...
    private void calculateColumnWidthValues(SectionData sd) {
        int availableWidth = mStickyLayoutManager.getWidth() - sd.contentStart - sd.contentEnd;
    ...
    private void layoutChild(LayoutState.View child, int top, int col, int rowHeight,
        ...
        final int height;
        if (child.getLayoutParams().height == StickyLayoutManager.LayoutParams.MATCH_PARENT) {
            height = rowHeight;
        } else {
            height = mStickyLayoutManager.getDecoratedMeasuredHeight(child.view);
        }
        final int width;

        if (col == mNumColumns - 1) {
            width = mStickyLayoutManager.getDecoratedMeasuredWidth(child.view);
        } else {
            width = Math.min(mColumnWidth, mStickyLayoutManager.getDecoratedMeasuredWidth(child.view));
        }
    ...
    private void measureChild(LayoutState.View child, SectionData sd) {
        ...
        mStickyLayoutManager.measureChildWithMargins(child.view,
                sd.marginStart + sd.marginEnd + widthOtherColumns,
                0);
    }

    public static class LayoutParams extends StickyLayoutManager.LayoutParams {
        ...
        public static LayoutParams from(ViewGroup.LayoutParams other) {
            if (other == null) {
                Log.w("SuperSLiM", "Null value passed in call to GridStickyLayoutManager.LayoutParams.from().");
